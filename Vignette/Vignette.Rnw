...
\section{Quick start}
The \verb|cuRe| package has two main functions for estimating parameteric cure models on the relative survival scale, namely \verb|FlexCureModel| and \verb|fit.cure.model|. In addition, functionalities for providing crude mortality measures are also included.

Load the \verb|cuRe| package and included dataset, \verb|colonDC| on 15564 colon cancer patients. The name \verb|colonDC|, indicates that the \verb|colon| dataset of the \verb|rstpm2| package is used after some "data cleaning".
<<message=F, warning=F>>=
library(cuRe)
data("colonDC")
s <- sample(nrow(colonDC), 500)
colonDC <- colonDC[s,]
head(colonDC)
@

Extract general population hazard at the observed follow-up times.
<<>>=
...
Fit flexible cure model.
<<>>=
fit <- FlexCureModel(Surv(FUyear, status2) ~ 1, data = colonDC,
                     bhazard = "bhaz", n.knots = 5)
@

Plot the relative survival, the exess hazard, and the survival of the uncured patients.
...
Compute the loss of lifetime and the conditional probability of eventually dying from other causes than cancer given survival until time $t$.
<<fig.height = 5, fig.width = 10, fig.align = "center">>=
ll <- calc.LL(fit, time = seq(0, 15, length.out = 100),
               rmap = list(age = agedays, sex = sex, year = dx))

prob <- calc.Crude(fit, time = seq(0, 15, length.out = 100),
                   type = "othertime",
                   rmap = list(age = agedays, sex = sex, year = dx), n = 5000)
...
R(t) = \frac{S(t)}{S^*(t)}
\end{equation}
are commonly used to describe the net disease-specific survival in a patient population. Here $S(\cdot)$ and $S^*(\cdot)$ are the all-cause and general population survival functions, respectively. The term "net" refers to the interpretation of the disease-specific survival which is the survival of the patients in a world where it is impossible to die from other causes than the disease. Non-parametric estimators of the relative survival curve have been available for many years.

The \verb|relsurv| package contains functions for modelling the relative survival non-parametricaly. For instance the immediate relative survival of the colon cancer patients may be computed usig the function \verb|rs.surv| in the following way.
<<message = F, warning = F, out.width = "70%", fig.align = "center">>=
library(relsurv)
fit <- rs.surv(Surv(FU, status2) ~ 1 + ratetable(age = agedays,
                                                 sex = sex,
                                                 year = dx),
               data = colonDC,
               ratetable = survexp.dk,
...
<<>>=
fit.cm <- fit.cure.model(Surv(FUyear, status2) ~ age,
                         data = colonDC,
                         bhazard = "bhaz",
                         formula.k1 = ~ age,
                         formula.k2 = ~ 1)
...
<<>>=
fit.cm.ln <- fit.cure.model(Surv(FUyear, status2) ~ age, data = colonDC,
                         bhazard = "bhaz", formula.k1 = ~ age,
                         formula.k2 = ~ 1,
                         dist = "lognormal")
...
rs5 <- sapply(rs5$res, function(x) x$Est)

rs5.ln <- predict(fit.cm.ln, time = 5, newdata = data.frame(age = ages))
rs5.ln <- sapply(rs5.ln$res, function(x) x$Est)

plot(rs5 ~ ages, type = "l", ylim = c(0, 1), ylab = "5 year relative survival")
lines(rs5.ln ~ ages, col = 2)
@
...
<<out.width = "70%", fig.align = "center">>=
fit.flex <- FlexCureModel(Surv(FUyear, status2) ~ age,
                           data = colonDC, n.knots = 5,
                           bhazard = "bhaz", smooth.formula = ~ age)
...
<<out.width = "70%", fig.align = "center">>=
fit.flex.time <- FlexCureModel(Surv(FUyear, status2) ~ age,
                           data = colonDC, n.knots = 5,
                           bhazard = "bhaz", n.knots.time = list(age = 3))
...
plot(pi ~ ages, data = pred_cure, type = "l", ylim = c(0, 1),
     ylab = "Cure rate", xlab = "Diagnostic age")
@
...
<<>>=
fit.pen <- pstpm2(Surv(FUyear, status2) ~ 1, data = colonDC,
                  bhazard = colonDC$bhaz,
                  smooth.formula=~s(log(FUyear)) + s(log(FUyear), by=age))
@
...
fit.null <- FlexCureModel(Surv(FUyear, status2) ~ 1, data = colonDC,
                          bhazard = "bhaz", n.knots = 5)

newdata <- data.frame(age = 60, sex = "male", year = 2010,
                      age_days = 50 * 365.24)

crude.null.cancer <- calc.Crude(fit.flex.time, newdata = newdata,
                                time = seq(0, 20, length.out = 100),
                                ci = T, rmap = list(age = age_days), n = 5000)

crude.null.other <- calc.Crude(fit.flex.time, newdata = newdata,
                               type = "other",
                               time = seq(0, 20, length.out = 100),
                               ci = T, rmap = list(age = age_days), n = 5000)

crude.null.prob <- calc.Crude(fit.flex.time, newdata = newdata,
                              type = "othertime",
                              time = seq(0, 20, length.out = 100),
                              ci = T, rmap = list(age = age_days), n = 5000)

crude.null.prob.reverse <- calc.Crude(fit.flex.time, newdata = newdata,
                                      type = "othertime",
                                      time = seq(0, 20, length.out = 100),
                                      reverse = T,
                                      ci = T, rmap = list(age = age_days), n = 5000)
...
<<fig.align = "center", out.width = "70%">>=
crude.time <- calc.Crude(fit.flex.time, type = "othertime",
                         newdata = data.frame(age = c(50, 60, 70),
                                              sex = "male",
                                              year = 2010,
                                              age_days = c(50, 60, 70) * 365.24),
                         time = seq(0, 20, length.out = 100), reverse = T,
                         ci = F, rmap = list(age = age_days), n = 5000)

crude.pen.time <- calc.Crude(fit.pen, type = "othertime",
                             newdata = data.frame(age = c(50, 60, 70),
                                                  sex = "male",
                                                  year = 2010,
                                                  age_days = c(50, 60, 70) * 365.24),
...
plot(crude.time, ci = F, col = 1)
plot(crude.pen.time, ci = F, add = T, col = 2)
@
